...
async def create_category_service(db: AsyncSession, payload: CategoryCreate) -> Category:
...
async def list_categories_service(db: AsyncSession, name: str | None = None, limit: int = 50, offset: int = 0,) -> list[Category]:
    return await CategoriesDAO.list(db=db, name=name, limit=limit, offset=offset,)


async def update_category_service(db: AsyncSession, category_id:int, payload: CategoryUpdate,) -> Category:
    category = await CategoriesDAO.get_by_id(db, category_id)
    if not category:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Category not found")
